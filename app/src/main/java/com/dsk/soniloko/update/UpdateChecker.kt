package com.dsk.soniloko.update
...
/**
 * Checks a `version.json` published on GitHub Releases (see .github/workflows/check-newpipe.yml)
 * and, if it's newer than the installed build, downloads and launches the system installer.
 * No Play Store involved — this is how the app updates itself outside of it.
 */
object UpdateChecker {
    // TODO: replace with your actual GitHub username/repo once created (see manual.md).
    private const val VERSION_JSON_URL =
        "https://raw.githubusercontent.com/dskmusic/soniloko/main/version.json"
    ...
    fun downloadAndInstall(context: Context, info: UpdateInfo) {
        runCatching {
            ...
            val dest = File(context.getExternalFilesDir(Environment.DIRECTORY_DOWNLOADS), "SoniLoko-update.apk")
            if (dest.exists()) dest.delete()

            val request = DownloadManager.Request(Uri.parse(info.apkUrl))
                .setTitle("SoniLoko — actualización")
